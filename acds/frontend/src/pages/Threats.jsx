...
const formatTime = (ts) =>
  new Date(ts).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) + 'Z';
  ...
  return (
    <td className="px-6 py-5" onClick={e => e.stopPropagation()}>
      {playbook ? (
        ...
        >
          {expanded ? 'Hide' : source === 'gemini' ? '⚡ View' : '📋 View'}
        </button>
      ) : (
        <button
          onClick={generate}
        ...
        >
          {loading ? (
            <span className="flex items-center gap-1">
              <span className="material-symbols-outlined text-[10px] animate-spin" style={{ verticalAlign: 'middle' }}>sync</span>
              ...
            </span>
          ) : '⚡ Playbook'}
        </button>
      ...
      {expanded && playbook && (
        <div className="absolute left-0 right-0 z-50 mt-1 mx-6 bg-[#080808] border border-[#A84B2B]/30 p-4 shadow-2xl max-h-48 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
          <p className="font-['IBM_Plex_Mono'] text-[10px] text-[#A84B2B] uppercase tracking-widest mb-2 flex items-center gap-2">
            ...
            {source === 'gemini' ? '⚡ Gemini AI Playbook' : '📋 Rule-Based Playbook'} — {alert.alert_id}
          </p>
...
// Deterministic severity row styles
const rowSeverityClass = (severity) => {
  if (severity === 'Critical') return 'hover:bg-[#93000a]/10 border-l-2 border-l-[#93000a]/60';
  if (severity === 'High')     return 'hover:bg-[#ff9800]/5 border-l-2 border-l-[#ff9800]/30';
  return 'hover:bg-[#1e110d]/50 border-l-transparent';
};
...
export default function Threats() {
  ...
  // Live-ticking sync timestamp
  useEffect(() => {
    const tick = () => {
      setSyncTime(
        new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) + 'Z'
      );
    };
    tick();
  ...
  // Track which alerts are "new" for animation
  const newAlertIds = useRef(new Set());
  useEffect(() => {
    const currentIds = new Set(alerts.map(a => a.alert_id));
    const fresh = new Set([...currentIds].filter(id => !prevAlertIds.current.has(id)));
    newAlertIds.current = fresh;
    prevAlertIds.current = currentIds;
    // Clear new-alert flag after animation completes
    if (fresh.size > 0) {
      const timer = setTimeout(() => { newAlertIds.current = new Set(); }, 600);
  ...
  return (
    <div className="pt-10 pb-20 px-8 min-h-screen relative overflow-y-auto bg-[#0A0C0E] text-[#e5e2e1]">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className={`bg-[#0A0C0E] p-6 border-l-4 ${colors.border} transition-colors duration-1000`}>
            ...
            <div className="font-['Space_Grotesk'] text-3xl font-bold">
              {threatStats.network_latency} <span className={`text-xs font-['IBM_Plex_Mono'] ${colors.text} uppercase`}>Avg</span>
            </div>
          </div>
          <div className={`bg-[#0A0C0E] p-6 border-l-4 ${colors.border} transition-colors duration-1000`}>
            <div className="font-['IBM_Plex_Mono'] text-[10px] text-[#6B6560] uppercase mb-1">QUEUE DEPTH</div>
            <div className="font-['Space_Grotesk'] text-3xl font-bold">
              {threatStats.queue_depth} <span className="text-xs font-['IBM_Plex_Mono'] text-[#6B6560] uppercase">Events</span>
            </div>
          </div>
          <div className="bg-[#0A0C0E] p-6 border-l-4 border-[#6B6560]/40">
            ...
            <div className="font-['Space_Grotesk'] text-3xl font-bold">
              {threatStats.active_detectors} <span className="text-xs font-['IBM_Plex_Mono'] text-neutral-600 uppercase">/ 4 ONLINE</span>
            </div>
          </div>
          <div className={`bg-[#0A0C0E] p-6 border-l-4 ${colors.border} transition-colors duration-1000`}>
            <div className="font-['IBM_Plex_Mono'] text-[10px] text-[#6B6560] uppercase mb-1">EVENT DENSITY</div>
            <div className="font-['Space_Grotesk'] text-3xl font-bold">
              {filteredAlerts.length} <span className={`text-xs font-['IBM_Plex_Mono'] ${colors.text} uppercase`}>Detected</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#0A0C0E] overflow-hidden">
          <div className="bg-[#353534] px-6 py-4 flex justify-between items-center border-b border-[#6B6560]/10">
            ...
            <span className="font-['IBM_Plex_Mono'] text-[10px] text-[#6B6560] tabular-nums">
              LAST SYNC: {syncTime}
            </span>
          </div>
              ...
              <tbody className="divide-y divide-[#6B6560]/5">
                {filteredAlerts.length === 0 ? (
                  <tr>
                    <td colSpan="8" className="px-6 py-16 text-center">
                      <div className="flex flex-col items-center gap-4">
                        <span
                          className="material-symbols-outlined text-[#6B6560]/30"
                          style={{ fontSize: '48px' }}
                        >
                          radar
                        </span>
                        <div className="font-['IBM_Plex_Mono'] text-[#6B6560] text-xs uppercase tracking-widest">
                          {activeTab === 'all' ? 'Awaiting Live Feed...' : `No ${activeTab} threats detected`}
                        </div>
                        <div className="flex gap-1">
                          {[0, 1, 2].map(i => (
                            <span
                              key={i}
                              className="w-1 h-1 rounded-full bg-[#A84B2B]/40 animate-pulse"
                  ...
                  filteredAlerts.map((a, i) => {
                    const isNew = newAlertIds.current.has(a.alert_id);
                    return (
                      <tr
                        key={a.alert_id || i}
                        className={`transition-all cursor-pointer group relative ${rowSeverityClass(a.severity)} ${isNew ? 'animate-[fadeIn_0.4s_ease-out]' : ''}`}
                        style={isNew ? { animation: 'fadeIn 0.4s ease-out' } : {}}
                      >
                        ...
                        <td className="px-6 py-5">
                          <span className={`font-['IBM_Plex_Mono'] text-[9px] px-2 py-0.5 font-bold uppercase tracking-tighter ${
                            a.severity === 'Critical'
                              ? 'bg-[#93000a] text-[#ffdad6]'
                              : a.severity === 'High'
                              ? 'bg-[#ff9800] text-[#000]'
                              : 'bg-[#353534] text-[#E8E2D9]'
                          }`}>
                            {a.severity?.toUpperCase()}
                          </span>
                        </td>
                        <td className="px-6 py-5 font-['IBM_Plex_Mono'] text-[10px] text-[#6B6560]">{a.mitre?.id || a.mitre_ttp || 'T1000'}</td>
                        <td className="px-6 py-5 font-['IBM_Plex_Mono'] text-xs text-[#E8E2D9] tabular-nums">{formatTime(a.timestamp)}</td>
      ...
      <style>{`
        @keyframes fadeIn {
          from { opacity: 0; transform: translateY(-4px); background: rgba(168,75,43,0.08); }
